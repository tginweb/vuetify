...
/* @vue/component */
  ...
  methods: {
    ...
    genYearItem (year: number): VNode {
      ...
      const active = !!this.value.find(date => parseInt(date, 10) === year)
    ...
    genYearItems (): VNode[] {
      const children = []
      const selectedYears = this.value.length ? this.value.map(date => parseInt(date, 10)) : [new Date().getFullYear()]
      const maxYear = Math.max(...selectedYears)
      const minYear = Math.min(...selectedYears)
      const lastYear = this.max ? parseInt(this.max, 10) : (maxYear + 100)
      const firstYear = Math.min(maxYear, this.min ? parseInt(this.min, 10) : (minYear - 100))
